...
for node, sensor_dists in distances_from_sensors.items():
    ...
    top3_results[node] = [s for s, _ in sorted_sensors[:3]]

# check if any node has less than 3 sensors
for node, info in top3_results.items():
    if len(info) < 3:
        print(f"Node {node} has less than 3 sensors: {info}")

# Save results to JSON
with open("3_closest_sensors.json", "w") as f:
