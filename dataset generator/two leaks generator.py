...
def generate_two_leaks_data(inp_file, leak_node_1, leak_node_2, leak_diam_mm_1, leak_diam_mm_2, duration_days, report_step_min, sensor_nodes):
    # -----------------------------
    # 1. Load the network
    # -----------------------------
    ...
    # -----------------------------
    # 2. Add leaks as emitters at specified nodes
    # -----------------------------
    for leak_node, leak_diam_mm in [(leak_node_1, leak_diam_mm_1), (leak_node_2, leak_diam_mm_2)]:
    ...
    # -----------------------------
    output_csv = f'Datasets\\Multiple leaks\\leakage_{leak_node_1}_{leak_diam_mm_1}_{leak_node_2}_{leak_diam_mm_2}.csv'
...
# Generate all possible unique unordered pairs (no self-pairs)
all_pairs = list(itertools.combinations(leak_nodes, 2))

# set total number of simulations to run
n_runs = 300

# Sample n distinct pairs
random.seed(42)  # for reproducibility
sampled_pairs = random.sample(all_pairs, n_runs)

# run simulation and generate data
start_time = time.time()
for node1, node2 in sampled_pairs:
    leak_diam_mm1 = random.choice([7, 11, 15, 19])  # leak diameter in mm
    leak_diam_mm2 = random.choice([7, 11, 15, 19])  # leak diameter in mm

    generate_two_leaks_data(inp_file, node1, node2, leak_diam_mm1, leak_diam_mm2, duration_days, report_step_min, sensor_nodes)
